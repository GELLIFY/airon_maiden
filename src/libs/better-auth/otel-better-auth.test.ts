...
import {
  ...
  SEMATTRS_USER_EMAIL,
  SEMATTRS_USER_ID,
} from "./otel-better-auth";

describe("otel-better-auth", () => {
  describe("instrumentBetterAuth", () => {
    it("should return unchanged if not an object", () => {
      expect(instrumentBetterAuth(null as any)).toBe(null as any);
      expect(instrumentBetterAuth(undefined as any)).toBe(undefined as any);
    });
    ...
    it("should instrument server instance with api methods", () => {
      ...
      expect((instrumented as any).__OtelBetterAuthInstrumented).toBe(true);
    });
    ...
    it("should not duplicate otel plugin if already provided", () => {
      ...
      const plugins = ((mockServer as any).options?.plugins ??
        []) as ReturnType<typeof otelPlugin>[];
