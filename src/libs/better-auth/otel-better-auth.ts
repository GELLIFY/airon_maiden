...
const DEFAULT_TRACER_NAME = "otel-better-auth";
const INSTRUMENTED_FLAG = "__OtelBetterAuthInstrumented" as const;
...
type AuthWithPlugins<TOptions extends BetterAuthOptions> = Auth<TOptions> & {
  options: TOptions & {
 ...
 */
function instrumentServer<TOptions extends BetterAuthOptions>(
...
function ensureOtelPlugin<TOptions extends BetterAuthOptions>(
  auth: Auth<TOptions>,
  config: InstrumentBetterAuthConfig & { tracer: Tracer },
): void {
  const authWithOptions = auth as AuthWithPlugins<TOptions>;
 ...
 * ```
 */
export function instrumentBetterAuth<TOptions extends BetterAuthOptions>(
  auth: Auth<TOptions>,
  ...
  if ((auth as Auth<TOptions> & InstrumentedAuth)[INSTRUMENTED_FLAG]) {
    return auth;
  }
  ...
  (auth as Auth<TOptions> & InstrumentedAuth)[INSTRUMENTED_FLAG] = true;
      ...
      after: [
        {
          ...
          handler: createAuthMiddleware(async (ctx) => {
            try {
              ...
              const spanKey = `${ctx.request?.method || "GET"}:${
                ctx.request?.url
              }`;
